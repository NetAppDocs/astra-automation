...
[.lead]
You need to generate an Astra API token to use the Astra Control REST API.

== Introduction

The Astra API token must be included with every REST API call. Note the following:

* You can generate an API token at the Astra web user interface.
* The token is carried in the `Authorization` HTTP request header.
...
When you access the Astra Control REST API through Curl or a programming language, you must include the API bearer token in the `Authorization` request header.
