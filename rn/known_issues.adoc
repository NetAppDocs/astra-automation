...
The Astra Data Store storage backend is in an `Unknown` state after issuing a call to retrieve. In this condition, the storage backend is actually available and can be communicated with. A component within the storage backend is likely in an unhealthy state and needs to be returned to a healthy state for the storage backend to show as `Available`.
