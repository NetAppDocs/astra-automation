...
This release includes an expansion and update of the REST API along with xxx

>> a change to the documentation structure to better support the evolution of Astra Control through the future release updates.

[TIP]
Review DS (Jira list) and AH emails. Also ACC 22.04 review site. Which have nothing to do with the API, and might be removed? Rather than remove, maybe place under one "additional enhancements" section.

=== Continued expansion of the REST API

xxx
>> Two new resource types
>> Version of 12 existing resources updated
