...
You can enable optimization for restic backups using the flag `useResticParallel` in the `astra.datamover` settings resource. When set to `true`, the number of backendConnections that restic uses is increased when backing up large volumes to an Azure container. For volumes of 1TB through 4TB, 128 connections are used. For volumes larger than 4TB, 512 backend connections are used.
