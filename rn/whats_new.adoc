...
Two new resources types have been added: *package* and *upgrade*. In addition, several of the version of several existing resources has been upgraded.

=== Enhanced RBAC with namespace granularity

When binding a role with the associated user, you can limit the cluster namespaces the user has access to. See the *Role Binding API* reference and the link:../workflows_infra/workflows_infra_before.html[Infrastructure workflows] and for more information.

=== Bucket removal

You can remove a bucket that is no longer needed or is not functioning properly.

=== Support for Cloud Volumes ONTAP

Cloud Volumes ONTAP can be added as a storage backend.
