...
Two new resources types have been added, including *package* and *upgrade*. In addition, several of the version of several existing resources has been upgraded.

=== Enhanced RBAC with namespace granularity

When binding or associating a role with a user, you can now limit the cluster namespaces that the user has access to. See the link:../workflows_infra/workflows_infra_before.html[Infrastructure workflows] and the *Role Binding API* for more information.

=== Bucket removal

You can now remove a bucket that is no longer needed or not functioning properly.

=== Support for Cloud Volumes ONTAP

Cloud Volumes ONTAP can now be added as a storage backend.
