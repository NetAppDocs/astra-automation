...
Perform the workflow link:../workflows_infra/wf_list_clusters.html[List the clusters] and select the clusters.

=== 3. List the storage classes for a specific cluster

Perform the following REST API call to list the storage classes for a specific cluster.
...
|Path
|GET
|/account/{account_id}/topology/v1/clouds/<CLOUD_ID>/clusters/<CLUSTER_ID>/storageClasses
|===
